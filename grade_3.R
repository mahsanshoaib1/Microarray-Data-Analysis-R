...
# extracting rows in which tumor grade is equal to 3 and tissue type if normal and saving it in a variable.
Grade_3 <- filter(sdrf, FactorValue..tumor.grade.== 3 | Characteristics..tissue.type.== "normal breast tissue")
unique(Grade_3$FactorValue..tumor.grade.) # checking unique values in the tumor grade columns
unique(Grade_3$Array.Data.File) 
count(Grade_3)
head(Grade_3)
...
# rownames(Grade_3) <- Grade_3$Array.Data.File # making Array.Data.File column as rownames
...
# Reading CEL FILES using Grade_3 data
Grade_3 <- AnnotatedDataFrame(Grade_3)
local_data <-oligo::read.celfiles(filenames = file.path(path,Grade_3$Array.Data.File), phenoData = Grade_3)
stopifnot(validObject(local_data))
colnames(pData(local_data))
rownames(pData(local_data))
# rownames(pData(local_data)) <- Grade_3$Array.Data.File # making Array.Data.File column as rownames
# rownames(pData(local_data))
...
## boxplot  
# ?boxplot
oligo::boxplot(local_data, target = "core", main="Grade 3 TNBC Raw Data")

setwd("D:/study/lab_projects/Dr_Zubair/Breast _cancer/practice_practical/Selected Datasets/outputs/grade_3")
...
# view(data_summary)
# write.csv(raw_data_summary, "data_summary_Grade_3_tnbc_raw.csv")


## histogram: Helps identify batch effects or abnormal distributions.
# oligo::hist(exprs(local_data))
hist(as.vector(exprs(local_data)), breaks = 100, main = "Intensity Distribution of Grade 3 Raw Data") 

## density plot
#affy::hist(local_data, target= "core", col = 1:length(sampleNames(local_data)), lty = 1, main = "Density Plot of Grade 3 TNBC Raw Data")
...
#plot(hc, main = "Hierarchical Clustering Dendrogram of Grade 3 TNBC Raw Data")
...
# box plot
oligo::boxplot(expr_matrix, target = "core", main="Grade 3 TNBC Normalized Data Quality Check")
...
#write.csv(normalized_data_summary, "normalized_data_summary_Grade_3_tnbc_raw.csv")

# histogram: Helps identify batch effects or abnormal distributions.
# oligo::hist(exprs(norm_data))
hist(as.vector(exprs(norm_data)), breaks = 100, main = "Intensity Distribution of Grade 3 TNBC Normalized Data") # giving the error due to size of data

#density plot
affy::hist(norm_data, col = 1:length(sampleNames(norm_data)), lty = 1, main = "Density Plot of Grade 3 TNBC Normalized Data")
...
#  ggtitle("PCA plot of the Grade 3 TNBC Normalized epxression data")+
...
#plot(hc, main = "Hierarchical Clustering Dendrogram of Grade 3 TNBC Normalized Data")
#
...
group <- factor(Grade_3$FactorValue..tissue.type.) # Normal vs Tumor
...
contrast_matrix <- makeContrasts(Grade3TNBC_vs_Normal = `triple.negative.breast.cancer..TNBC.` - `normal.breast.tissue`, levels = design)
...
deg_results <- topTable(fit2, coef = "Grade3TNBC_vs_Normal", number = Inf, adjust.method = "fdr")

# Save the results
# write.csv(deg_results, "Grade_3_TNBC_DEG_results.csv", row.names = TRUE)
...
write.csv(DEG_norm_DS, "updated_Grade_3_TNBC_DEG_results.csv")
...
ggplot(deg_results, aes(x = logFC, y = -log10(adj.P.Val), color = Significance)) +
  ...
  theme_minimal() +
  ggtitle("Volcano Plot of Differentially Expressed Genes of Grade 3 TNBC") +
...
pheatmap(dists, col = hmcol,
         ...
         main = "Improved Clustering Heatmap of Grade 3 TNBC Differentially expressed genes")
